...
class GithubRepoMapAdmin(admin.TabularInline):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name in ["code_repo", "documentation_repo"]:
            github_app_installation_id = request.resolver_match.kwargs.get(
                "object_id")
            kwargs["queryset"] = models.GithubRepository.objects.filter(
                owner_id=github_app_installation_id)
        return super(GithubRepoMapAdmin, self).formfield_for_foreignkey(db_field, request, **kwargs)
...
class GithubAppInstallationAdmin(admin.ModelAdmin):
    list_display = ("account_name", "created_at", )
    readonly_fields = ("account_name", "installation_id",
                       "account_type", "state", "creator")
    ...
    def get_form(self, request, obj=None, **kwargs):
        form = super(GithubAppInstallationAdmin, self).get_form(
            request, obj, **kwargs)
...
admin.site.register(models.CodeRepoPullRequest)
admin.site.register(models.DocumentationRepoPullRequest)
