...
class GithubInstallationManager:
    ...
    def get_jwt_headers(cls) -> dict:
        private_key = default_backend().load_pem_private_key(settings.APP_AUTH_KEY, None)
    ...
    def get_installation_access_token(self) -> Tuple[str, datetime.datetime]:
        ...
        if response.ok:
            ...
            try:
                return (response_data['token'], datetime.datetime.strptime(response_data['expires_at'], "%Y-%m-%dT%H:%M:%S%z"))
    ...
    def get_installation_details(self):
        ...
        response = requests.get(
            f"https://api.github.com/app/installations/{self.installation_id}", headers=headers)
        if response.ok:
            response_data = response.json()
    ...
    def _get_repository_page(self, page: int, per_page: Optional[int] = None):
        ...
        response = requests.get(
            f"https://api.github.com/user/installations/{self.installation_id}/repositories?per_page={per_page}&page={page}", headers=headers)
        if response.ok:
            response_data = response.json()
            logger.info(response.content.decode())
            return response_data
    ...
    def generate_check_suite_instance(self):
        ...
        response = requests.post(
            f"https://api.github.com/repos/sharmashobhit/django-wordpress/check-runs", headers=headers, json=data)
        logger.info(response.content.decode())
        pass
